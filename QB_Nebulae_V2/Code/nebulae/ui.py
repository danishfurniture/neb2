...
############ Begin use reset LEDs in other instruments (danishfurniture)
			# Requires this line in 'nebconfigbegin' area of instr code: ResetLEDAtEOL,1

            else:
	            if "ResetLEDAtEOL" in self.controlhandler.configData:
		            self.ResetLED = int(self.controlhandler.configData.get("ResetLEDAtEOL")[0])
	            else:
		            self.ResetLED = 0

	            if any( [ (self.currentInstr == "a_granularlooper") , (self.ResetLED == 1) ] ): # If default instrument or "ResetLEDAtEOL" =1 (danishfurniture)
...
########### Begin use red speed LEDs during recording (danishfurniture)
            ...
            if "RecordRedLEDs" in self.controlhandler.configData:
	            self.recordLED = int(self.controlhandler.configData.get("RecordRedLEDs")[0])
            else:
	            self.recordLED = 0

            if any( [ (self.currentInstr == "a_granularlooper") , (self.recordLED == 1) ] ): # If default instrument or if "RecordRedLEDs" set in 'nebconfigbegin' # End use red speed LEDs during recording (danishfurniture)
